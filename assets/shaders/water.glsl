...
vec3 hash3( vec3 p )
{
	...
	return fract(
        sin(
            vec3(
                dot(p,vec3(127.1,311.7, 121.0)),
                dot(p,vec3(269.5,183.3,7.12)),
                dot(p,vec3(112.6, 5.231, 201.2))
            )
        )*43758.5453
    );
}
...
{
    vec3 n = floor(uvw.xyz);
    vec3 f = fract(uvw.xyz);
